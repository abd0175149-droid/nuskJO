<template>
    <AppLayout>
        <template #header>كشف حساب العميل: {{ client.name }}</template>
        <div class="space-y-6">
            <!-- Client Info Card -->
            <div class="dash-card p-5">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mobile-form-grid">
                    ...
                    <div><span class="text-gray-400 text-xs">الذمة الحالية</span><p class="font-bold font-mono text-lg" dir="ltr" :class="parseFloat(client.balance_jod)>=0?'text-green-500':'text-red-500'">{{ Math.abs(Number(client.balance_jod)).toLocaleString('en',{minimumFractionDigits:3}) }} JOD</p></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="dash-card p-4 text-center"><p class="text-xs text-gray-400">الرصيد الختامي</p><p class="font-bold font-mono text-lg mt-1" dir="ltr" :class="parseFloat(client.balance_jod)>=0?'text-green-500':'text-red-500'">{{ fmtAbs(client.balance_jod) }}</p></div>
            </div>
                ...
                <table class="w-full text-sm responsive-table">
                    ...
                    <tbody>
                        <tr v-for="(e, index) in entriesWithBalance" :key="e.id" class="border-t border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                            ...
                            <td data-label="مدين (ذمة)" class="px-4 py-3 text-right font-mono text-xs" dir="ltr" :class="parseFloat(e.debit)>0?'text-red-500 font-bold':'text-gray-300 dark:text-gray-600'">{{ parseFloat(e.debit)>0?Number(e.debit).toFixed(3):'—' }}</td>
                            <td data-label="دائن (تسديد)" class="px-4 py-3 text-right font-mono text-xs" dir="ltr" :class="parseFloat(e.credit)>0?'text-green-500 font-bold':'text-gray-300 dark:text-gray-600'">{{ parseFloat(e.credit)>0?Number(e.credit).toFixed(3):'—' }}</td>
...
const applyFilter = () => { router.get('/clients/'+props.client.id,{from:from.value,to:to.value},{preserveState:true,replace:true}); };
...
const entriesWithBalance = computed(() => {
    ...
    return (props.entries || []).map(e => {
        // العميل مدين (Debit يزيده، Credit ينقصه)
        currentBalance += (parseFloat(e.debit || 0) - parseFloat(e.credit || 0));
        return {
            ...e,
