        ...
        <div class="space-y-6">
            ...
            <div class="dash-card p-5">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mobile-form-grid">
                    ...
                    <div><span class="text-gray-400 text-xs">الذمة الحالية</span><p class="font-bold font-mono text-lg" dir="ltr" :class="parseFloat(summary?.current_balance)>=0?'text-green-500':'text-red-500'">{{ Math.abs(Number(summary?.current_balance||0)).toLocaleString('en',{minimumFractionDigits:3}) }} JOD</p></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="dash-card p-4 text-center"><p class="text-xs text-gray-400">الرصيد الختامي</p><p class="font-bold font-mono text-lg mt-1" dir="ltr" :class="parseFloat(summary?.current_balance)>=0?'text-green-500':'text-red-500'">{{ fmtAbs(summary?.current_balance) }}</p></div>
            </div>
